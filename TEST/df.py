...
npart = [4] # Número de partições em cada direção
...
if len(npart) == 2:
    ...
    for i in range (0, ((npart[1] + 1) * (npart[0] + 1)) - (npart[0] + 1)):
...
print(f'Tamanho lista de valores iniciais: {len(initial_values)}')

discvar = ['x'] # Variáveis de discretização
dvar = ['T'] # Variáveis dependentes
eq = 'dT/dt = 0.1*d2T/dx2 ' # Equação diferencial parcial (EDP)


resultadodisc = df(eq, dvar, discvar, npart)     
