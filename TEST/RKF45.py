...
def SERKF45(oldexpr, ivar, dvars, initial_values, x0, xn, n):
    olddvar = symbol_references(dvars)
    oldivar = symbol_references(ivar)
    expr = [parse_expr(oldexpr[i]) for i in range(len(oldexpr))]
        
    yn = initial_values.copy() # Cópia dos valores iniciais
