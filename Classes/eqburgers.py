...
PDE1 = PDE.PDE('dF/dt = (-F)*dF/dx + 0.0002*d2F/dx2',
...
resultado = df(
    ...
    west_func_bd="0",  # x=0
    west_bd="Dirichlet",
    method="forward",  # MUDANÇA: Método explícito para evitar instabilidade inicial
...
resultado_final_cn = CN_copy.cn_1d_optimized(
    resultado[0],
    resultado[1],
    nt=1000,
    dt=0.001,
    u_init_array=np.full(len(resultado[1]), 1.0)
)
...
def analytical(x, t, alpha=0.0002):
    u = np.zeros_like(x)
